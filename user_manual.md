...
1. Laden Sie eine Kopie des TIM-Projekts von [hier](https://github.com/FB-EnergyTransition/TIM) herunter.
2. _Optional/Empfohlen:_ Speichern Sie die CSV-Datei, aus der Sie Daten hochladen möchten, im Projektordner "resources" ab.
3. Öffnen Sie ein Terminalfenster (je nach Python-Installation Anaconda Prompt oder cmd etc.) und navigieren Sie zum Ordner, in dem das Projekt enthalten ist.
4. Falls noch nicht vorhanden, stellen Sie die Verbindung zum Proxmox-Server her, in dem die InfluxDB läuft (siehe Kapitel "Einrichten der Verbindung zum Proxmox-Server").
5. Führen Sie die main.py Datei aus. Dazu ist folgender Befehl erforderlich:
...
Bei Option 2 werden die Voraussetzungen ausgegeben und Sie erhalten erneut die Möglichkeit zur Auswahl zwischen diesen beiden Optionen.

7. Wenn Sie Option 1 wählen, werden Sie nach dem Dateipfad gefragt, in dem die hochzuladende CSV-Datei liegt.
Geben Sie diesen Pfad ein. Den Ausgangspunkt dafür bildet der Ordner, in dem das TIM-Projekt liegt.
...
9. Anschließend folgt die Frage, in welchen Bucket die Dateien aus der Datei hochgeladen werden sollen.
...
11. Im nächsten Schritt werden Sie je nach Ihrer gewählten Option aufgefordert, entweder die Einheit für alle Werte einzugeben
oder die Einheit für jede Wertespalte wird einzeln abgefragt.
